...
#define TASK_BUT2_STACK_SIZE (2048 / sizeof(portSTACK_TYPE))
#define TASK_BUT2_STACK_PRIORITY (tskIDLE_PRIORITY)
...
static void BUT2_init(void);
void pin_toggle(Pio *pio, uint32_t mask);
static void USART1_init(void);
void LED_init(int estado);

volatile delayTicks = 2000;
/************************************************************************/
...
static void task_but(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (xSemaphoreTake(xSemaphoreBut, 500)) {
      /* atualiza frequencia */
      delayTicks -= 100;
	...
	if (xSemaphoreTake(xSemaphoreBut2, 500)) {
		/* atualiza frequencia */
		delayTicks += 100;
...
int main(void) {
  ...
  if (xTaskCreate(task_but, "BUT2", TASK_BUT2_STACK_SIZE, NULL,
				  TASK_BUT2_STACK_PRIORITY, NULL) != pdPASS) {
	  printf("Failed to create UartTx task 2\r\n");
  }
